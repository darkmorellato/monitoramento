...
import { currentBase64Image, loadImageFile, clearImage, extractDataFromImage, extractFromClipboard, handleDragOver, handleDragLeave, handleDrop } from './ocr.js';
...
window.__updateAll = (newLogs) => { logs = newLogs; updateAll(); };
// Provê acesso à loja atual para outros módulos sem permitir sobrescrita direta
...
form.addEventListener('submit', async e => {
    ...
    try {
        ...
        // Primeiro salvamos o log principal (inclui upload ou fallback de imagem)
        await saveLogEntry(newEntry, currentStore);
        
        // Agora atualizamos os outros registros em um lote atômico (Firestore Batch)
        const batch = getDB().batch();
        let hasUpdates = false;
        
        tempLogs.forEach(l => {
            if (l.id !== newEntry.id) {
                const docRef = getDB().collection('Lojas').doc(currentStore.name).collection('registros').doc(String(l.id));
                batch.update(docRef, { diff: l.diff, pct: l.pct });
                hasUpdates = true;
            }
        });
